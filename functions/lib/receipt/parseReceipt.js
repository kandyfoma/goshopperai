...
exports.parseReceipt = functions
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      const isUnlimited =
        config_1.config.app.trialScanLimit === -1 ||
  ...
  .https.onCall(async (data, context) => {
    ...
    try {
      ...
      const parsedResults = await Promise.all(
        images.map(img => parseWithGemini(img, mimeType)),
      );
...
async function updateUserStats(userId, receipt) {
  ...
  try {
    ...
    let stats =
      userDoc.exists &&
      ((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.stats)
        ? userDoc.data().stats
        : {
            totalScans: 0,
            totalSpent: 0,
    ...
    stats.itemsScanned =
      ...
      (((_b = receipt.items) === null || _b === void 0 ? void 0 : _b.length) ||
        0);
    // Calculate XP
    const xpEarned =
      10 +
      Math.min(
        ((_c = receipt.items) === null || _c === void 0 ? void 0 : _c.length) ||
          0,
        10,
      ); // Base XP + items bonus
